...
const Demo: React.FC = () => {
  ...
  // Auto-advance logic based on completed fields
  useEffect(() => {
    // Step 1: Contact Information - check if all required fields are filled
    if (formData.name && formData.email && formData.company && formData.phone) {
      setCurrentStep(prev => Math.max(prev, 2));
    }
    
    // Step 2: Demo Type Selection - check if demo type and industry are selected
    if (formData.demoType && formData.industry) {
      setCurrentStep(prev => Math.max(prev, 3));
    }
    
    // Step 3: Pain Points - check if business description and current challenges are filled
    if (formData.businessDescription && formData.currentChallenges) {
      setCurrentStep(prev => Math.max(prev, 4));
    }
    
    // Step 4: Specializations - check if at least one specialization is selected
    if (formData.rmeSpecializations && formData.rmeSpecializations.length > 0) {
      setCurrentStep(prev => Math.max(prev, 5));
    }
    
    // Step 5: Configuration - check if team size and consultation package are selected
    if (formData.teamSize && formData.consultationPackage) {
